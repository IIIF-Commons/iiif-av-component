...
function mockElements(elements: any[]) {
  const list: Array<{ play: Mock; pause: Mock; buffering: boolean; paused: boolean }> = [];
...
describe('Composite media', () => {
  test('creating composite media from canvases', async () => {
    ...
    const [composite] = compositeMediaFromCanvases(manifest.getSequenceByIndex(0).getCanvases(), {});
