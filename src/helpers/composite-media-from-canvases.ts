...
export function compositeMediaFromCanvases(canvases: Canvas[], options: { adaptiveAuthEnabled?: boolean }) {
  ...
  for (const canvas of canvases) {
    ...
    for (const annotation of annotations) {
      ...
      mediaElement.setSize(
        ...
        convertToPercentage(mediaSource.width || canvas.getWidth(), canvas.getWidth()),
        convertToPercentage(mediaSource.height || canvas.getHeight(), canvas.getHeight())
      );
