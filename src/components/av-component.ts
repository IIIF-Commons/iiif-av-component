...
export class AVComponent extends BaseComponent {
    ...
    if (this._data.virtualCanvasEnabled) {
      for (let i = 0; i < this.canvasInstances.length; i++) {
        ...
        if (currentCanvasId) {
          ...
          if (
            ((canvasInstance.isVirtual() || this.canvasInstances.length === 1) && currentCanvasId === canvasId) ||
  ...
  private _getPosterImageCss(expanded: boolean): any {
    ...
    if (currentCanvas) {
      ...
      const containerWidth: number = currentCanvas.$playerElement.parent().width();
      const containerHeight: number = currentCanvas.$playerElement.parent().height() - ($options.height() as number);
