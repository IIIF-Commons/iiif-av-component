...
var CanvasInstance_1 = require("./CanvasInstance");
...
var AVComponent = /** @class */ (function (_super) {
    ...
    AVComponent.prototype.set = function (data) {
        ...
        if (diff.includes("range") && this._data.range) {
            ...
            if (!range) {
                ...
                var canvasId = Utils_1.AVComponentUtils.getFirstTargetedCanvasId(range);
                if (canvasId) {
                    ...
                    if (canvasInstance) {
                        ...
                        if (this._data.canvasId &&
                            (this._data.canvasId.includes("://")
                                ? manifesto_js_1.Utils.normaliseUrl(this._data.canvasId)
    ...
    AVComponent.prototype._reset = function () {
        ...
        if (this._data && this._data.helper && this._data.helper.manifest) {
            ...
            if (behavior && behavior.toString() === vocabulary_1.Behavior.AUTO_ADVANCE) {
                var virtualCanvas_1 = new VirtualCanvas_1.VirtualCanvas();
    ...
    AVComponent.prototype._initCanvas = function (canvas) {
        var _this = this;
        var canvasInstance = new CanvasInstance_1.CanvasInstance({
        ...
        // }, false);
        canvasInstance.on(CanvasInstance_1.CanvasInstanceEvents.PREVIOUS_RANGE, function () {
            _this._prevRange();
        }, false);
        canvasInstance.on(CanvasInstance_1.CanvasInstanceEvents.NEXT_RANGE, function () {
            _this._nextRange();
        }, false);
        canvasInstance.on(Events.RANGE_CHANGED, function (rangeId) {
            _this.fire(Events.RANGE_CHANGED, rangeId);
        }, false);
        canvasInstance.on(VolumeControl_1.VolumeEvents.VOLUME_CHANGED, function (volume) {
            _this._setCanvasInstanceVolumes(volume);
            _this.fire(VolumeControl_1.VolumeEvents.VOLUME_CHANGED, volume);
        }, false);
        canvasInstance.on(CanvasInstance_1.CanvasInstanceEvents.PLAYCANVAS, function () {
            _this.fire(CanvasInstance_1.CanvasInstanceEvents.PLAYCANVAS);
        }, false);
        canvasInstance.on(CanvasInstance_1.CanvasInstanceEvents.PAUSECANVAS, function () {
            _this.fire(CanvasInstance_1.CanvasInstanceEvents.PAUSECANVAS);
        }, false);
